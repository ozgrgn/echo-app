<script lang="ts">
	import { CATEGORIES, gpiZone, type CategoryScore, type CategoryKey } from '@revora/review-core';
	...
	const filteredSorted = $derived.by(() => {
		...
		if (attentionOnly) {
			list = list.filter((cs) => cs.gpi < 80);
		}
		switch (sortKey) {
			case 'gpi_desc':
				list.sort((a, b) => b.gpi - a.gpi);
				break;
			case 'gpi_asc':
				list.sort((a, b) => a.gpi - b.gpi);
				break;
			case 'trend_desc':
			...
			case 'department':
				list.sort((a, b) =>
					CATEGORIES[a.category].department.localeCompare(CATEGORIES[b.category].department, 'tr')
				);
				break;
		}
		return list;
	...
	const attentionCount = $derived(data.hotelScore.categoryScores.filter((cs) => cs.gpi < 80).length);
	...
	function zoneBarClass(gpi: number): string {
		const z = gpiZone(gpi);
	...
	// Sentiment distribution helper — returns percentages summing to 100
	function distribution(cs: CategoryScore): { pos: number; neg: number; neu: number } {
		const total = cs.positiveCount + cs.negativeCount + cs.neutralCount;
		if (total === 0) return { pos: 0, neg: 0, neu: 0 };
		return {
			pos: (cs.positiveCount / total) * 100,
			neg: (cs.negativeCount / total) * 100,
			neu: (cs.neutralCount / total) * 100
		};
	}
</script>
	...
	<header class="flex flex-wrap items-center justify-between gap-3">
		<div>
			<p class="text-sm text-text-2">
				10 kategori · <span class="font-mono">{data.hotelScore.gpi.toFixed(1)}</span> ortalama GPI ·
					...
					<header class="flex items-start justify-between gap-3">
						<div class="flex-1 min-w-0">
							...
							<p class="text-xs text-text-3 mt-0.5 truncate">
								Sorumlu: <span class="text-text-2">{meta.department}</span>
							</p>
						</div>
						<div class="text-right shrink-0">
							<div class="text-3xl font-bold {zoneClass(cs.gpi)}">{cs.gpi.toFixed(1)}</div>
					...
					<div>
						<div class="flex h-2 rounded-full overflow-hidden bg-surface-2">
							<div
								...
								style="width: {dist.pos}%"
								title="{cs.positiveCount} olumlu"
							></div>
							<div
								class="bg-text-3 transition-all duration-500"
								style="width: {dist.neu}%"
								title="{cs.neutralCount} nötr"
							></div>
							<div
								class="bg-danger transition-all duration-500"
								style="width: {dist.neg}%"
								title="{cs.negativeCount} olumsuz"
							></div>
						</div>
						<div class="flex justify-between text-xs text-text-3 mt-1.5">
							<span><span class="text-success font-semibold">{cs.positiveCount}</span> olumlu</span>
							<span><span class="text-text-2">{cs.neutralCount}</span> nötr</span>
							<span><span class="text-danger font-semibold">{cs.negativeCount}</span> olumsuz</span>
						</div>
					</div>
