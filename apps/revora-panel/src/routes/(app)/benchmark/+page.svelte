<script lang="ts">
	import { CATEGORIES, gpiZone, rpiLabel, type CategoryKey } from '@revora/review-core';
	...
	<section class="bg-surface-1 border border-border rounded-lg p-5 shadow-sm">
		...
		<ul class="space-y-2.5">
			{#each allRows as row (row.venueSlug)}
				<li
					class={[
						'grid grid-cols-[auto_1fr_auto] gap-3 items-center p-3 rounded-md transition-colors',
				...
				>
					...
					<div class="flex items-center gap-2 min-w-[14rem]">
						{#if row.isOwn}
							<span class="text-warning font-bold" title="Sizin oteliniz">▶</span>
						{/if}
						<span class="text-sm font-medium text-text-1 truncate" title={row.venueName}>
							{row.venueName}
						</span>
			...
			<!-- Market average row -->
			<li
				class="grid grid-cols-[auto_1fr_auto] gap-3 items-center p-3 rounded-md bg-surface-2/20 border border-dashed border-text-3/20"
			>
				<div class="flex items-center gap-2 min-w-[14rem]">
					<span class="text-text-3">—</span>
					<span class="text-sm italic text-text-2">Pazar Ortalaması</span>
				</div>
				<div class="h-3 rounded-full bg-surface-2 overflow-hidden relative">
					<div
