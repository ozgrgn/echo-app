<script lang="ts">
	...
	// Yanıtsız yorum sayısı — responseStats.rate'den türetildi
	const unansweredCount = $derived(
		Math.round(data.hotelScore.reviewCount * (1 - data.hotelScore.responseStats.rate))
	);
	const unansweredZone = $derived(
		unansweredCount === 0 ? 'green' : unansweredCount <= 5 ? 'yellow' : 'red'
	);
	const unansweredColorClass = $derived(
		unansweredZone === 'green'
			? 'text-success'
			: unansweredZone === 'yellow'
				? 'text-warning'
				: 'text-danger'
	);
	const unansweredBorderClass = $derived(
		unansweredZone === 'green'
			? 'border-l-success'
			: unansweredZone === 'yellow'
				? 'border-l-warning'
...
<div class="space-y-6">
	...
	<div class="grid grid-cols-2 lg:grid-cols-5 gap-4">
		...
		<a
			...
			class="bg-surface-1 border border-border rounded-lg p-5 border-l-4 {unansweredBorderClass} shadow-sm hover:shadow-md transition-shadow block"
			title="Cevap bekleyen yorumlara git"
		>
			<div class="text-xs text-text-2 uppercase tracking-wider mb-1">Yanıtsız Yorum</div>
			<div class="text-3xl font-bold {unansweredColorClass}">
				{unansweredCount}
			</div>
			<div class="text-xs text-text-3 mt-1">cevap bekliyor →</div>
		</a>
			...
			{#each data.hotelScore.categoryScores as cs (cs.category)}
				...
				>
					<header class="flex items-center justify-between">
						...
						<span class="text-xl font-bold {zoneClass(cs.headlineScore)}">{cs.headlineScore.toFixed(1)}</span>
					</header>
					...
					<div class="h-1.5 rounded-full bg-surface-2 overflow-hidden">
						<div
							...
							class:bg-success={gpiZone(cs.headlineScore) === 'green'}
							class:bg-warning={gpiZone(cs.headlineScore) === 'yellow'}
							class:bg-danger={gpiZone(cs.headlineScore) === 'red'}
							style="width: {Math.min(100, Math.max(0, cs.headlineScore))}%"
						></div>
					</div>
