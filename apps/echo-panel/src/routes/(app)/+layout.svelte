	...
	>
		...
		<div class="px-6 pb-4 border-b border-white/10 mb-4">
			...
			{:else}
				...
				{#if auth.venueName ?? data?.session?.venueName}
					<span class="block text-xs text-sidebar-text mt-2 truncate">{auth.venueName ?? data?.session?.venueName}</span>
				{/if}
			{/if}
		</div>
		...
		<div class="px-6 pt-4 border-t border-white/10 text-xs space-y-3">
			{#if !collapsed}
				<div>
					...
					<div class="truncate" title={(auth.tenantKey ?? data?.session?.tenantKey) ?? undefined}>{auth.tenantKey ?? data?.session?.tenantKey}</div>
				</div>
				<button
					onclick={logout}
