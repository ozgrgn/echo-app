<script lang="ts">
	...
	const zone = $derived(gpiZone(data.categoryScore.headlineScore));

	// Derived sentiment totals for the KPI tiles (5-bucket → 2-way summary).
	const dist = $derived(data.categoryScore.distribution);
	const negSum = $derived(dist.strongNegativeCount + dist.negativeCount);
	const posSum = $derived(dist.positiveCount + dist.strongPositiveCount);
	...
	const gaugeOffset = $derived(gaugeC * (1 - Math.min(100, Math.max(0, data.categoryScore.headlineScore)) / 100));
...
<div class="space-y-6">
	...
	<div
		class="bg-surface-1 border border-border rounded-lg p-6 border-l-4 {zoneBorderClass(
			data.categoryScore.headlineScore
		)} shadow-sm"
	>
		<div class="grid grid-cols-1 md:grid-cols-[auto_1fr] gap-6 items-center">
			...
			<div class="flex flex-col items-center justify-center">
				<svg width={gaugeSize} height={gaugeSize} viewBox="0 0 {gaugeSize} {gaugeSize}">
					...
					>
						{data.categoryScore.headlineScore.toFixed(1)}
					</text>
					<text
						x={gaugeSize / 2}
						y={gaugeSize / 2 + 30}
			...
			<div class="space-y-4 min-w-0">
				<div>
					...
					<p class="text-sm text-text-2 mt-1">
						Sorumlu departman:
						<span class="font-medium text-text-1">{meta.primaryOwner}</span>
				...
				<div class="grid grid-cols-2 sm:grid-cols-4 gap-2 text-sm">
					...
					<div class="rounded-md bg-success-light p-3">
						<div class="text-xs text-success/80 mb-0.5">Olumlu ifade</div>
						<div class="font-bold text-success">{posSum}</div>
					</div>
					<div class="rounded-md bg-danger-light p-3">
						<div class="text-xs text-danger/80 mb-0.5">Olumsuz ifade</div>
						<div class="font-bold text-danger">{negSum}</div>
					</div>
					<div class="rounded-md bg-surface-2 p-3">
				...
				<!-- Full polarity histogram (5 bars) -->
				<div class="rounded-md bg-surface-2 p-4 mt-2">
					<div class="text-xs text-text-3 uppercase tracking-wider mb-2">
						Duygu dağılımı
					</div>
					<PolarityHistogram
						distribution={data.categoryScore.distribution}
						variant="bars"
						height={70}
					/>
				</div>
